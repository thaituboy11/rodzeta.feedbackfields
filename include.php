...
EventManager::getInstance()->addEventHandler("main", "OnAdminTabControlBegin",
	function (&$form) {
		if ($GLOBALS["APPLICATION"]->GetCurPage() != "/bitrix/admin/message_edit.php") {
	  	return;
	  }
	  ob_start();
	...
	?>
	  ...
	  <tr>
			...
			<td class="adm-detail-content-cell-r" width="50%">
				<input name="rodzeta_options_csv_save" value="Y" type="checkbox"
					<?= $optionsCsv["save"] == "Y"? "checked" : "" ?>>
			</td>
		</tr>
		...
		<tr valign="top">
			...
			<td class="adm-detail-content-cell-r" width="50%">
				...
				<table width="100%" class="js-table-autoappendrows">
					<tbody>
						<?php foreach (AppendValues($optionsCsv["fields"], 5, "") as $i => $fieldCode) { ?>
							<tr data-idx="<?= $i ?>">
								<td>
									<input name="rodzeta_options_csv_fields[<?= $i ?>]" type="text" placeholder="Код поля"
